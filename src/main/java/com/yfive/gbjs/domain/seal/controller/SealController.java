...
public interface SealController {
  ...
  @GetMapping
  @Operation(summary = "전체 띠부씰 조회", description = "등록된 모든 띠부씰 목록을 조회합니다.")
  ResponseEntity<ApiResponse<SealListResponse>> getAllSeals();

  @GetMapping("/users/{userId}")
  @Operation(summary = "회원 띠부씰 조회", description = "특정 회원의 띠부씰 수집 현황을 조회합니다. (수집한 것/수집하지 않은 것 포함)")
  ResponseEntity<ApiResponse<UserSealListResponse>> getUserSeals(
  ...
  ResponseEntity<ApiResponse<SealProductListResponse>> getSealProducts();
}