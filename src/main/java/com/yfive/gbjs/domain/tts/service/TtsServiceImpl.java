...
public class TtsServiceImpl implements TtsService {
  ...
  @Override
  public TtsResponse convertTextToSpeech(
    ...
    if (gender != null && gender.equals(Gender.MALE)) {
      if (speechType != null && speechType.equals(SpeechType.A)) {
        type.append("C");
      } else {
        type.append("D");
      }
    } else if (speechType != null) {
      type.append(speechType);
    }
    ...
    try {
      ...
      if (response != null && response.getAudioContent() != null) {
        ...
        return TtsResponse.builder().audioUrl(s3Url).build();
      } else {
