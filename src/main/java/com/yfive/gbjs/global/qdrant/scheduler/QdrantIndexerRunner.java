...
public class QdrantIndexerRunner implements CommandLineRunner {
  ...
  private void verifyData() {
    try {
      ...
      CollectionInfo collectionInfo = qdrantClient.getCollectionInfoAsync(collectionName).get();
      ...
      if (pointCount == 0) {
        log.warn(
            "⚠️  Warning: 0 points found. This might mean the source database tables are empty.");
      }
    } catch (Exception e) {
